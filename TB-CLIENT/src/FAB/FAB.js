...
const FAB = props => {
  ...
  const styles = createStyles(isDriver,theme)
  ...
  const _onPanHandlerStateChange = useAnimatedGestureHandler({
    ...
    onEnd: _ => {
      console.log(fabPositionX.value + FAB_WIDTH);
      console.log(width / 2);

      if (fabPositionX.value + FAB_WIDTH > width / 2) {
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1, }}>
      <DrawerComp toggleFab={(status) => setisFabVisible(status)} />

      {isFabVisible && <PanGestureHandler onHandlerStateChange={_onPanHandlerStateChange}>
        <Animated.View style={[styles.rootStyles, animatedRootStyles]}>
          ...
          <TapGestureHandler onHandlerStateChange={_onTapHandlerStateChange}>
            <Animated.View style={[styles.fabButtonStyles, animatedFABStyles,]}>
              ...
              {/* <SubButton onPress={() => Alert.alert('Pressed 1!')} label="1" size={25} color={isDarrkMode ? 'white' : 'black'} icon={"wheel"} /> */}

              <Animated.Image tintColor={ 'white' } style={{ height: 30, width: 30 }} source={isDriver ? require('../asserts/images/steetingwheel.png') : require('../asserts/images/hand.png')}>
...
const createStyles = (isDriver, theme) => StyleSheet.create({
  rootStyles: {
